...
class RocketSimulator:
    def __init__(self):
        ...
        self.reference_area = 0.01
        self.air_density = 1.225
        self.winglet_effectiveness = 0.2  # INCREASED from 0.1
        self.dt = 0.01
        self.gravity = np.array([0, 0, -9.81])
        self.thrust_curve = self._generate_thrust_curve()
        self.moment_of_inertia = np.array([0.002, 0.002, 0.004])
        self.time = 0.0

    def _generate_thrust_curve(self):
        times = np.array([0, 0.05, 0.5, 1.0, 1.5, 2.0, 2.5])
        thrusts = np.array([60, 55, 40, 30, 20, 10, 0])  # 60N > 4.9N weight
    ...
    def step(self, state, winglet_commands):
        # IMPROVED: Faster winglet response
        new_winglet_positions = 0.8 * state.winglet_positions + 0.2 * winglet_commands  # Changed from 0.9/0.1
    ...
    def calculate_reward(self, state, target_altitude=100.0):
        """Improved reward - less harsh penalties, better shaping"""
        altitude_reward = state.position[2] * 0.2  # Increased from 0.1
        
        # FIXED: Much gentler tilt penalty to allow learning
        tilt_penalty = -(abs(state.orientation[0]) + abs(state.orientation[1])) * 2  # Reduced from 10
        
        # Reduced drift penalty
        drift_penalty = -(state.position[0]**2 + state.position[1]**2) * 0.005  # Reduced from 0.01
        
        # ADDED: Progressive altitude bonus
        if state.position[2] >= target_altitude:
            altitude_bonus = 100  # Increased reward
        elif state.position[2] >= target_altitude * 0.5:
            altitude_bonus = 25   # Intermediate reward
        else:
            altitude_bonus = 0
        
        # Crash penalty
        crash_penalty = -500 if state.position[2] < 0 else 0  # Reduced severity
...
class RLRocketController:
    ...
    def _build_model(self):
        """Policy network with better initialization"""
        inputs = keras.layers.Input(shape=(10,))  # Keep original 10 inputs
        
        # Better initialization for stability
        hidden1 = keras.layers.Dense(32, activation='relu', 
                                    kernel_initializer='he_normal')(inputs)
        hidden2 = keras.layers.Dense(16, activation='relu',
                                    kernel_initializer='he_normal')(hidden1)
        
        # Action means
        action_means = keras.layers.Dense(4, activation='tanh', 
                                        kernel_initializer='glorot_normal',
                                        name='action_means')(hidden2)
        
        # More conservative log std initialization
        log_stds = keras.layers.Dense(4, activation='tanh',
                                    kernel_initializer='zeros',
                                    bias_initializer=keras.initializers.Constant(-1.0),  # Start with lower variance
                                    name='log_stds')(hidden2)
    ...
    def train(self, episodes=EPISODES):
        """Fixed training loop - single stepping only"""
        ...
        for episode in range(episodes):
            ...
            while simulator.time < 3.0:
                # FIXED: Get sensors from current state without stepping
                sensors = self._get_current_sensors(state, simulator)
    ...
    def _get_current_sensors(self, state, simulator):
        """Get current sensor readings without advancing simulation"""
        # Calculate current forces and acceleration
        local_thrust = np.array([0, 0, 1]) * simulator.thrust_curve(simulator.time)
        thrust = simulator._rotate_vector(local_thrust, state.orientation)
        drag = -0.5 * simulator.air_density * simulator.drag_coefficient * \
            simulator.reference_area * np.linalg.norm(state.velocity) * state.velocity
        
        acceleration = (thrust + drag) / simulator.mass + simulator.gravity
        
        return {
            'accelerometer': simulator._rotate_vector(acceleration, state.orientation),
